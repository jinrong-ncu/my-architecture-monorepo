...
const columns: ProColumnType<UserItem>[] = [
    ...
    },
    {
        ...
        valueType: 'operation',
        operation: [
            { label: '编辑', onClick: (r: UserItem) => message.info(`编辑: ${r.name}`) },
            { label: '授权', disabled: (r: UserItem) => !r.status, onClick: (r: UserItem) => message.info(`授权: ${r.name}`) },
            { label: '删除', onClick: (r: UserItem) => message.error(`删除: ${r.name} - ${r.id}`) },
        ],
    },
];
